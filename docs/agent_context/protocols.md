...
| **WebSocket**     | 开发调试直连          | 仅开发调试 | `third_party/xiaozhi-esp32/docs/websocket.md`    |
| **MCP**           | 设备控制              | 两种模式通用 | `third_party/xiaozhi-esp32/docs/mcp-protocol.md` |
...
参考 `third_party/xiaozhi-esp32-server/main/xiaozhi-server/core/` 目录：
- `handle/` - 连接处理和消息分发
- `providers/` - ASR/LLM/TTS 提供者

### 网关模式实现

参考 `third_party/xiaozhi-mqtt-gateway/` 目录：
...
参考 `third_party/mcp-endpoint-server/src/` 目录：
